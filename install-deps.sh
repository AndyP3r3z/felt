...
# THIS WORKS ON FEDORA 43.
sudo dnf install -y gtk3-devel cairo-devel gtk-layer-shell-devel glib2 gobject-introspection-devel python3-devel python-pip python3-gobject python3-cairo python3-loguru pkgconf cmake meson ninja xkeyboard-config-devel libxkbfile-devel iso-codes-devel pulseaudio-libs-devel libdbusmenu-gtk3
	...
	if ls /usr/lib/girepository-1.0/Cvc-1.0.typelib 1> /dev/null 2>&1; then
		...
		if [ "$answer" != "${answer#[Yy]}" ]; then
			...
			if [ "$answer2" != "${answer2#[Yy]}" ]; then
				sudo rm -rf /usr/lib64/girepository-1.0/Cvc-1.0.typelib
				sudo rm -rf /usr/share/gir-1.0/Cvc-1.0.gir
				sudo rm -rf /usr/lib64/libcvc.s*
			fi
		else
			exit 0
		fi
...
install_gir() {
	...
	meson install
	cd libcvc
	sudo cp -r libcvc.s* /usr/lib64/
	sudo cp Cvc-1.0.typelib /usr/lib64/girepository-1.0/
